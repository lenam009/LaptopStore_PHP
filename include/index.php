...
$date = DateTime::createFromFormat('Y-m-d H:i:s', '2023-05-20 19:32:30');
//TRẢ VỀ HIỆU CỦA 2 DATETIME (THAM SỐ TRẢ VỀ Y 2 THẰNG TRÊN)
$diff = $date->diff($dateCurrent);
//TRẢ VỀ TỔNG NGÀY CHÊNH LỆCH
//echo $diff->days;
//TRẢ VỀ GIỜ(KO QUÁ 24H) BI LỆCH
//echo $diff->format("%H");
//TRẢ VỀ KHOẢNG CÁCH NGÀY (GIỚI HẠN TRONG 1 THÁNG)
//echo $diff->format("%r%d");
//TRẢ VỀ KHOẢNG CÁCH NĂM
//echo $diff->format("%r%y")."<br/>";
//TRẢ VỀ KHOẢNG CÁCH THÁNG (GIỚI HẠN TRONG 1 NĂM)
//echo $diff->format("%r%m");


//GÁN PAGE VỚI TRƯỜNG HOP PAGE < 1 && CHƯA CÓ PAGE
$page = $_GET['page'] ?? 1;
$page = $page < 1 ? 1 : $page;
//PHÂN TRANG
$productEachPage = 3;
    ...
    <div class="row mt-2">
        <div class="col-12 text-right">
            <?php if (isset($_GET['idType'])) : $idType = $_GET['idType'] ?>
                <a class="btn btn-secondary" href="../include/index.php?action=decPrice&idType=<?= $idType ?>">Sort decrease price</a>
                <a class="btn btn-secondary" href="../include/index.php?action=incPrice&idType=<?= $idType ?>">Sort increase price</a>
            <?php elseif (isset($_GET['search'])) : $search = $_GET['search'] ?>
                <a class="btn btn-secondary" href="../include/index.php?action=decPrice&search=<?= $search ?>">Sort decrease price</a>
                <a class="btn btn-secondary" href="../include/index.php?action=incPrice&search=<?= $search ?>">Sort increase price</a>
            <?php else : ?>
                <a class="btn btn-secondary" href="../include/index.php?action=decPrice">Sort decrease price</a>
                <a class="btn btn-secondary" href="../include/index.php?action=incPrice">Sort increase price</a>
            <?php endif; ?>
        </div>
                    ...
                    <div class="col-4 text-center mt-2  p-1 ">
                        <div class="card text-center border border-dark" style="height: 100%;">
                            <div class="card-body  card-hover">
                                <a href="../include/product.php?id=<?= $product->idProduct ?>"><img src="../include/images/<?= $product->imageProduct ?>" width="300" height="180" /></a><br /><br />
            ...
            <?php if ($i > 0) : ?>
                ...
                <?php else : ?>
                    ...
                    elseif (isset($_GET['idType'])) : ?>
                        <a class="text-primary btn" href="../include/index.php?page=<?= $i ?>&idType=<?= $_GET['idType'] ?>"><?= $i ?></a>
                    <?php //TRƯỜNG HỢP SORT
                    elseif (isset($_GET['action'])) : ?>
                        <a class="text-primary btn" href="../include/index.php?page=<?= $i ?>&action=<?= $_GET['action'] ?>"><?= $i ?></a>
                    <?php else : ?>
                        <a class="text-primary btn" href="../include/index.php?page=<?= $i ?>"><?= $i ?></a>
                    <?php endif; ?>
